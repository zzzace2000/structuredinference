...
class DKF(BaseModel, object):
    ...
    def _createGenerativeParams(self, npWeights):
        ...
        MU_COV_INP = DIM_HIDDEN_TRANS if self.params['transition_layers'] > 0 else \
            self.params['dim_stochastic']+self.params['dim_actions']
        ...
        emis_prev_layer = self.params['dim_hidden'] if self.params['emission_layers'] > 0 else self.params['dim_stochastic']
        if self.params['data_type'] == 'binary':
            npWeights['p_emis_W_ber'] = self._getWeight((emis_prev_layer, self.params['dim_observations']))
            npWeights['p_emis_b_ber'] = self._getWeight((self.params['dim_observations'],))
        elif self.params['data_type'] == 'gaussian':
            npWeights['p_emis_W_mu'] = self._getWeight((emis_prev_layer, self.params['dim_observations']))
            npWeights['p_emis_b_mu'] = self._getWeight((self.params['dim_observations'],))
            npWeights['p_emis_W_cov'] = self._getWeight((emis_prev_layer, self.params['dim_observations']))
            npWeights['p_emis_b_cov'] = self._getWeight((self.params['dim_observations'],))
        else:
    ...
    def _createInferenceParams(self, npWeights):
        ...
        # No need embedding since we are doing in Gaussian
    ...
    def _createLSTMWeights(self, npWeights):
        ...
        for suffix in suffices_to_build:
            for l in range(self.params['rnn_layers']):
                # Don't use embedding!!
                if l == 0:
                    npWeights['W_lstm_'+suffix+'_'+str(l)] = self._getWeight((DIM_INPUT,RNN_SIZE*4))
    ...
    def _getTransitionFxn(self, z, A=None):
        ...
        hid = z
        if self.params['dim_actions'] > 0:
            assert A != None, 'Need to specify Action!!!'
    ...
    #"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""#    
    def _buildLSTM(self, embedding, dropout_prob = 0.):
        """
    ...
    def _inferenceLayer(self, hidden_state):
        ...
        # List because LR, needs to add left and right
        if type(hidden_state) is list:
            eps        = self.srng.normal(size=(hidden_state[0].shape[1], hidden_state[0].shape[0],
                                                 self.params['dim_stochastic']))
        else:
            eps        = self.srng.normal(size=(hidden_state.shape[1], hidden_state.shape[0],
                                                self.params['dim_stochastic']))
    ...
    def _inferenceAndReconstruction(self, X, I, A, dropout_prob = 0.):
        ...
        # No need to do embedding
        # embedding          = self._qEmbeddingLayer(X)

        hidden_state       = self._buildLSTM(X, dropout_prob)
        ...
        # [bs x T x dim_stochastic] Mean equal to 0 only in t = 0
    ...
    def _getNegCLL(self, obs_params, X, M, I = None, batchVector = False):
        ...
        def log_likelihood(tgt, mu, cov):
            return (-(np.float32(0.5 * np.log(2 * np.pi)) + 0.5 * np.log(cov))
                         - 0.5 * T.sqr(tgt - mu) / cov)
        bce    = -log_likelihood(X, mean_p, cov_p)
        ...
        if M != None:
            ...
            negCLL = (bce.sum(2)).sum(1, keepdims=True)
    ...
    def _buildModel(self):
        ...
        self.setData       = theano.function([newX,newIndicators,newActions,newMask],None,
                updates=[(self.dataset,newX),(self.mask,newMask),
                         (self.indicators,newIndicators),(self.actions,newActions)])
        self.dimData       = theano.function([],[self.dataset.shape,self.indicators.shape,
                                                 self.actions.shape, self.mask.shape])
        ...
        if not self.params['validate_only']:
            ...
            self.train_debug         = theano.function(fxn_inputs,
                                                       [train_cost, norm_list[0], norm_list[1],
        ...
        self.getModelWeights = theano.function([], self.tWeights, name='Model Weights')
    #"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""# 
if __name__=='__main__':
    ...
    params['dim_actions']  = 0
    params['dim_indicators']  = 0
